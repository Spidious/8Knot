...
def cont_attrib_query(self, repos):
    """
    (Worker Query)
    ...
    query_string = f"""
                    WITH repo_list AS (
                                        SELECT repo_id 
                                        FROM (VALUES ({'), ('.join([str(r) for r in repos])})) AS r (repo_id)
                                    ), 
                        cntrb_by_repo AS (
                                        SELECT DISTINCT cmt.repo_id, 
                                                        cntrb.*,
                                                        cmt.cmt_author_date as created
                                        FROM augur_data.commits cmt
                                        INNER JOIN augur_data.contributors cntrb
                                        ON cmt.cmt_ght_author_id = cntrb.cntrb_id
                                        AND cmt.repo_id IN (SELECT repo_id FROM repo_list)
                                    )
                    SELECT c.repo_id AS id, 
                        c.created as created,
                        CASE WHEN c.cntrb_company IS NULL THEN 'volunteer' ELSE 'sponsored' END AS employment, 
                        COUNT(*) AS count
                    FROM cntrb_by_repo c
                    INNER JOIN repo_list r
                    ON c.repo_id = r.repo_id
                    GROUP BY c.repo_id, employment, c.created
                """
